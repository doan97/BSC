...
class Agent(object):
    def __init__(
        ...
        if lr is not None:
            ...
            self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimiser, mode='min', factor=0.5, patience=10, verbose=True, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=0, eps=1e-08)
    ...
    '''
    def predict_future(self, from_step, input_t):
        ...
        if c.INPUT_SENSOR_DIM > 0:
            ...
            if c.USE_SENSOR_SENSITIVITY:
                ...
                sensor_sensitivities_target = calc_sensor_sensitivities(self.sensor_directions, target_delta_position[-1])
        ...
        if(input_t > 0):
            ...
            position_delta_indices = (torch.tensor(0),torch.tensor(0),torch.tensor(np.array(range(c.POSITION_DIM_START,c.POSITION_DIM_END))))
            sensor_indices = (torch.tensor(0),torch.tensor(0),torch.tensor(np.array(range(c.SENSOR_DIM_START,c.SENSOR_DIM_END))))
                ...
                if not np.linalg.norm(delta_position) == 0.:
                    for i in range(c.INPUT_SENSOR_DIM):
                        ...
                        sensor_sensitivities[i] = 0.5 * (np.dot(direction, delta_position) /
                            (np.linalg.norm(direction) * np.linalg.norm(delta_position)) + 1.)
        ...
        position_delta, position, velocity, acceleration = self.sim.update(previous_sim_velocity, previous_sim_position, motor_commands, self, from_step+input_t)
    ...
    def actinf(self, from_step, to_step, learning_rate, velinf=False, vel_clamp=False):
        ...
        if c.MODE == 81:
            # To use an SCV a target position must be given. Since there is no target in mode 81, take the current position as target
        ...
        if c.INPUT_SENSOR_DIM > 0:
            ...
            if c.USE_SCV is True and self.seek_proximity is False:
                ...
                # Calculate SCV for each timestep and each sensor
    ...
    def real_step(self, from_step):
        ...
        sim_position_delta, sim_position, velocity, sim_acceleration = self.sim.update(previous_sim_velocity, previous_sim_position, motor_data_np, self, from_step)
    ...
    def learning_mini_epoch(self, from_step, to_step, num_time_steps):
        ...
        loss_sensors = c.SENSOR_WEIGHT_LEARNING * self.mse(sensor_predictions, sensor_targets) if c.OUTPUT_SENSOR_DIM > 0 else torch.tensor(0.)
        loss_accelerations = c.ACCELERATION_WEIGHT_LEARNING * self.mse(acceleration_predictions, acceleration_targets) if c.OUTPUT_ACCELERATION_DIM > 0 else torch.tensor(0.)
