...
def actinf(compare=True):

    faulthandler.enable()

    # CONSTANTS
    NUM_ALL_STEPS = 1000  # Amount of epochs of size NUM_MINI_EPOCHS
    if compare:
        NUM_ALL_STEPS = int(sys.argv[4])
    NUM_TIME_STEPS = 10
    TARGET_CHANGE_FREQUENCY = 50
    ...
    if c.MODE == 10:
        ...
        a2 = Agent(
            ...
            init_pos=np.array([-0.01, 0.8]),
            gui=gui, sim=sim,
            modelfile=comp_model_path
        )
        ...
        def set_targets(from_step):
            ...
            if (from_step == 0):
                if compare:
                    target_pos = np.array(target_positions[position_counter % 4])
                    a1.create_target(1,NUM_TIME_STEPS, target_pos)
                else:
...
position_counter = 0
target_positions = [[0.5,0.5],[2.5,0.5],[2.5,2.5],[0.5,2.5]]
