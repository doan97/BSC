...
class Simulator(object):
    def __init__(self, mode=0, maxthrust=1.0, leftborder=-1.5, rightborder=1.5, topborder=2.0, delta_time=1./30., stopwatch=None):
    ...
    def update(self, previous_velocity, previous_position, motorCommands, current_agent, time_step):
        ...
        if not isinstance(motorCommands, np.ndarray) or not (len(motorCommands.shape) == 1 and motorCommands.shape[0] == 4):
            raise ValueError("MotorCommands must be a 1-dimensional numpy-Array with 4 elements: shape = (4,) but shape is " + str(motorCommands.shape))
        ...
        new_velocity, new_position = self.check_borders(hypvel, hyppos, current_agent.radius, current_agent.other_agents, time_step)
        ...
        for t in range(time_steps):
            ...
            for ray_index in range(c.INPUT_SENSOR_DIM):
                ...
                if c.BORDER_PROXIMITY_WEIGHT > 0:
                    ...
                    proximity = calc_border_intersection_proximity(my_pos, my_radius, border_left, self.sensor_dirs[ray_index])
                    if (proximity is not None) and (ray_proximities[ray_index] < proximity):
                        ray_proximities[ray_index] = proximity
                    
                    # Right border
                    border_right = np.array([[1.5, 0], [1.5, 2]])
                    proximity = calc_border_intersection_proximity(my_pos, my_radius, border_right, self.sensor_dirs[ray_index])
                    if (proximity is not None) and (ray_proximities[ray_index] < proximity):
                        ray_proximities[ray_index] = proximity
                
                    # Top border
                    border_top = np.array([[-1.5, 2], [1.5, 2]])
                    proximity = calc_border_intersection_proximity(my_pos, my_radius, border_top, self.sensor_dirs[ray_index])
                    if (proximity is not None) and (ray_proximities[ray_index] < proximity):
                        ray_proximities[ray_index] = proximity
                    
                    # Bottom border
                    border_bottom = np.array([[-1.5, 0], [1.5, 0]])
                    proximity = calc_border_intersection_proximity(my_pos, my_radius, border_bottom, self.sensor_dirs[ray_index])
                    if (proximity is not None) and (ray_proximities[ray_index] < proximity):
                        ray_proximities[ray_index] = proximity
            ...
            if np.any(sensor_data[t]):
                ...
                if c.POINT_SPREAD is True:
                    ...
                    for sensor_index in range(c.INPUT_SENSOR_DIM):
                        point_spread_signal_tmp = point_spread(sensor_index, sensor_data[t, sensor_index], c.POINT_SPREAD_TYPE)
        ...
        for t in range(time_steps):
            ...
            for a in agent.other_agents:
                if a.data.positions.curr_idx > (from_step + t) % a.data.positions.length :
                    ...
                    active_sensor, proximity = self.calculate_sensor_input(my_pos, other_pos, agent.radius, other_radius)
    ...
    def simulate_multiple(self, previous_position, previous_velocity, all_motor_commands, current_agent, from_step):
        ...
        for motor_commands in all_motor_commands:
            # Perform update
            sensor_output, next_sensor_input, new_velocity, _ = self.update(previous_velocity, previous_position, motor_commands, current_agent, from_step)
