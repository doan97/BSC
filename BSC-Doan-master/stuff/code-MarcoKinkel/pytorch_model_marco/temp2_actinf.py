...
def actinf():
    ...
    if c.MODE == -1:
        a1 = Agent(id='A', color='red', init_pos=np.array([0., 1.]), gui=gui, sim=sim,
                   modelfile='./saves/mode_A_final.pt')
    ...
    if c.MODE == 0:
        a1 = Agent(id='A', color='red', init_pos=np.array([0., 1.]), gui=gui, sim=sim,
                   modelfile='./saves/mode_A_final.pt')
        a2 = Agent(id='B', color='green', init_pos=np.array([-1., 1.]), gui=gui, sim=sim,
                   modelfile='./saves/mode_A_final.pt')
    ...
    if c.MODE == 3:
        ...
        a1 = Agent(id='A', color='red', init_pos=pos_a1, gui=gui, sim=sim, modelfile='./saves/mode_A_final.pt',
                   stopwatch=s)
    ...
    if c.MODE == 4:
        a1 = Agent(id='A', color='red', position_loss_weight=0., init_pos=np.array([0., 1.]), gui=gui, sim=sim,
                   modelfile='./saves/mode_A_final.pt')
        a2 = Agent(id='B', color='green', init_pos=np.array([-1., 1.]), gui=gui, sim=sim,
                   modelfile='./saves/mode_A_final.pt')
    ...
    if c.MODE == 5:
        a1 = Agent(id='A', color='red', position_loss_weight=0., seek_proximity=False, init_pos=np.array([-1.4, 1.]),
                   gui=gui, sim=sim, modelfile='./saves/mode_A_final.pt')
        a2 = Agent(id='B', color='green', position_loss_weight=0., seek_proximity=True, init_pos=np.array([-1., 1.]),
                   gui=gui, sim=sim, modelfile='./saves/mode_A_final.pt')
    ...
    if c.MODE == 6:
        ...
        a1 = Agent(id='A', color='red', init_pos=init_pos, gui=gui, sim=sim, modelfile='./saves/mode_A_final.pt',
                   stopwatch=s)
        agents.append(a1)
        a1.gui_att.show_simulated_positions = True
    ...
    if c.MODE == 71:
        ...
        a1 = Agent(id='A', color='red', init_pos=agent_positions[scenario], show_sensor_plot=False, gui=gui, sim=sim,
                   modelfile='./saves/mode_A_final.pt')
        a1.gui_att.show_simulated_positions = False
            ...
            if (from_step == 0):
                ...
                if (a1.on_target_steps < 1 and a1.target_steps_total < TARGET_CHANGE_FREQUENCY):
                    ...
                    if scenario == 4:
                        ...
                        np.savetxt("./results/" + filename + ".csv", np.asarray(a1.performances2D)[:, -1, :],
                                   delimiter=";")
                        sys.exit(0)
    ...
    if c.MODE == 72:
        ...
        a1 = Agent(id='A', color='red', init_pos=agent_positions[a_scenario], gui=gui, sim=sim,
                   modelfile='./saves/mode_A_final.pt')
        a1.gui_att.show_simulated_positions = False

        a2 = Agent(id='B', color='lightgreen', init_pos=agent_positions[b_scenario], gui=gui, sim=sim,
                   modelfile='./saves/mode_A_final.pt')
        a2.gui_att.show_simulated_positions = False
        a2.gui_att.show_target = False
    ...
    if c.MODE == 73:
        ...
        a2 = Agent(id='B', color='lightgreen', init_pos=agent_positions[b_scenario], clamp_target_velocity_value=0.005,
                   gui=gui, sim=sim, modelfile='./saves/mode_A_final.pt')
        a2.gui_att.show_simulated_positions = False
        a2.gui_att.show_target = False
        a2.gui_att.show_scv_targets = False
    ...
    if c.MODE == 74:
        ...
        a2 = Agent(id='B', color='lightgreen', init_pos=b_pos, clamp_target_velocity_value=0.005, gui=gui, sim=sim,
                   modelfile='./saves/mode_A_final.pt')
        a2.gui_att.show_simulated_positions = False
        a2.gui_att.show_target = False
        a2.gui_att.show_scv_targets = False
    ...
    if c.MODE == 8:
        for i in range(24):
            ...
            a = Agent(id='A' + str(i), color=color, init_pos=np.array([x, y]), is_obstacle=True, gui=gui, sim=sim,
                      modelfile='./saves/mode_A_final.pt', stopwatch=s)
    ...
    if c.MODE == 81:
        for i in range(12):
            ...
            color = list(mcd.CSS4_COLORS.values())[i]
            a = Agent(id='A' + str(i), color=color, init_pos=np.array([x, y]), is_obstacle=True, seek_proximity=False,
                      show_sensor_plot=False, gui=gui, sim=sim, modelfile='./saves/mode_A_final.pt', stopwatch=s)
                ...
                if (a1.on_target_steps < 10):
                    ...
                    for o in obstacles:
                        ...
                        if 'linear' in scenario:
                            ...
                            o.path_radius = np.random.rand() * 0.5 + 0.1
                            o.path_angle = np.random.rand() * np.pi * 2
                            o.path_center = o.init_pos - np.array(
                                [np.math.cos(angle), np.math.sin(angle)]) * o.path_radius
    ...
    if c.MODE == 10:
        ...
        a2 = Agent(
            ...
            init_pos=np.array([-0.01, 0.8]),  # davon sind motorcommands2 [0, 1.25]), #m3 schoener! [-0.01, 0.8]),
            gui=gui, sim=sim,
            modelfile='./compare_models/v_noise_0.3'
        )
    ...
    if c.MODE == 11:
        ...
        a1 = Agent(id='A', color='red', init_pos=agent_positions[a_scenario], gui=gui, sim=sim,
                   modelfile='./saves/mode_B_final.pt')
        a1.gui_att.show_simulated_positions = False
        a1.gui_att.show_predictions = False
