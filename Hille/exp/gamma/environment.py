...
class Environment:
    # TODO stuff here
    def __init__(self, env_config, gui_config, seed=0):
        """
        ...
        self.gui_active = env_config['gui active']

        self.max_thrust = env_config['max thrust']
        self.left_border = env_config['left border']
        self.right_border = env_config['right border']
        self.top_border = env_config['top border']
        self.bot_border = env_config['bot border']
        ...
        self.delta_time = env_config['delta time']

        self.number_of_problems = env_config['number of problems']
        self.min_motor_value = env_config['min motor value']
        self.max_motor_value = env_config['max motor value']

        self.point_spread_flag = env_config['point spread']
        self.point_spread_size = env_config['point spread size']
        self.point_spread_type = env_config['point spread type']
        self.point_spread_sigma = env_config['point spread sigma']

        self.point_spread_clip_min = env_config['point spread clip min']
        self.point_spread_clip_max = env_config['point spread clip max']
        self.point_spread_round_decimals = \
            env_config['point spread round decimals']

        self.normalize_sensor_readings = env_config['normalize sensor readings']

        self.max_distance = env_config['max distance']
        self.border_proximity_weight = env_config['border proximity weight']

        self.position_dimensions = env_config['position dimensions']
        self.sensor_reading_dimensions = env_config['sensor reading dimensions']
        self.motor_command_dimensions = env_config['motor command dimensions']
        ...
        self.sensor_directions = \
            util.calc_sensor_directions(self.sensor_reading_dimensions)

        self.thrustDirections = util.calc_thrust_directions()
        self.tmp_counter = 0

        self.number_of_obstacles = env_config['number of obstacles']

        print(env_config['positions of obstacles'], type(env_config['positions of obstacles']))

        if env_config['positions of obstacles'] != 'None':
            assert \
                len(env_config['positions of obstacles']) \
                == self.number_of_obstacles, \
                'please provide exactly one position for each obstacle ' \
                'or set None'

        self.positions_of_obstacles = env_config['positions of obstacles']

        if isinstance(env_config['radii of obstacles'], list):
            assert \
                len(env_config['radii of obstacles']) \
                == self.number_of_obstacles, \
                'please provide exactly one radius for each obstacle ' \
                'or only one float value'

        self.radii_of_obstacles = env_config['radii of obstacles']
        ...
        self.max_obstacle_start_velocity = \
            env_config['max absolute obstacle start velocity']
        self.obstacle_line_acceleration_factor = \
            env_config['obstacle line acceleration factor']

        self.max_absolute_obstacle_rotation_radius = \
            env_config['max absolute obstacle rotation radius']
        self.obstacle_curve_acceleration_factor = \
            env_config['obstacle curve acceleration factor']

        self.default_object_mass = env_config['default object mass']
        ...
        if self.gui_active:
            if self.position_dimensions == 2:
                self.gui = gui.GUI(gui_config,
                                   np.array([self.right_border - self.left_border,
    ...
    def get_action_space(self):
        ...
        """
        return self.motor_command_dimensions
    ...
    def get_state_space(self):
        ...
        """
        return [self.position_dimensions, self.position_dimensions, self.sensor_reading_dimensions]
    ...
    def create_agent(self, agent_info):
        ...
        self.env_objects.append({
            ...
            'thruster activity': np.zeros(self.motor_command_dimensions),
            'sensor readings': np.zeros(self.sensor_reading_dimensions),
    ...
    def create_target_iterator_for_one_agent(self, agent_object,
        ...
        elif agent_object['target type'] == 'target of other':
            other_agent = None
        ...
        elif agent_object['target type'] == 'no proximity':
            # TODO implement this in targets.py if this is even possible there
            agent_object['target iterator'] = targets.ProximityTarget(None, target_lifetime,
    ...
    def render(self, arguments=None, sleep=0.0):
        """
    ...
    def get_thrusts_from_motor_commands(self, motor_commands):
        ...
        thrusts = np.zeros(self.motor_command_dimensions)
    ...
    def calculate_ray_distances(self, current_position, current_radius):
        ...
        ray_distances = np.zeros(self.sensor_reading_dimensions)

        for ray_index in range(self.sensor_reading_dimensions):
    ...
    def apply_point_spread(self, current_agent_sensor_readings):
        ...
        """
        point_spread_signal = np.zeros(self.sensor_reading_dimensions)

        for active_sensor_index in range(self.sensor_reading_dimensions):
            point_spread_signal += \
                util.point_spread(
                    active_sensor_index,
