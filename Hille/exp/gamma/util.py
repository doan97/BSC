...
def initialize(config_file):
    ...
    initialize_logging(log_location=config['default']['log location'],
                       log_level=config['default']['log level'])

    logging.info('configuration from file {} : {}'.format(config_file, config))
...
def initialize_logging(log_location, log_level):
    """
    TODO docstring
    ...
    if log_level == 'debug':
        ...
        raise ValueError('log level {} not available'.format(log_level))
    logging.basicConfig(format='%(asctime)s %(levelname)s : %(message)s',
                        datefmt='%m/%d/%Y %I:%M:%S',
                        filename='{}log.log'.format(log_location),
                        level=level)


def _save_pickle(file_name, object_):
    """
    TODO docstring
...
def save(file_name, epoch, environment_, agent_handler):
    """
    TODO docstring
    ...
    """
    gui = environment_.gui
    environment_.gui = None
    _save_pickle(file_name, {'epoch': epoch,
                             'environment': environment_,
                             'agents': agent_handler})
    environment_.gui = gui
...
def load(file_name):
    ...
    """
    data = _load_pickle(file_name)
    return data['epoch'], data['environment'], data['agents']
...
def get_train_info(config):
    ...
    """
    return \
        config['train']['number of epochs'], \
        config['train']['number of mini epochs'], \
        config['train']['number of time steps']


def get_setup(file_name, load_flag, mode, config, seed):
    """
    TODO docstring
    ...
    """
    if load_flag and os.path.isfile(file_name):
        epoch, environment_, agent_handler = load(file_name)
    else:
        epoch = 0
        environment_ = get_environment(config, seed)
        agent_handler = get_agents(mode, config,
                                   environment_.get_action_space(),
                                   environment_.get_state_space(),
                                   seed)

    num_epoch, num_mini_epoch, num_time_steps = \
        get_train_info(config)
    return \
        epoch, epoch + num_epoch, \
        environment_, agent_handler, \
        num_mini_epoch, num_time_steps


def get_agents(mode, config_file,
               action_space, state_space, seed):
    """
    TODO docstring
    ...
    """
    agent_handler = handler.Handler()
    if mode == -1:
        agent_handler.append(
        ...
        logging.error(' mode {} todo'.format(mode))
    elif mode == 2:
        logging.error(' mode {} todo'.format(mode))
    elif mode == 3:
        logging.error(' mode {} todo'.format(mode))
        ...
        logging.error('scenario mode {} not available')

    return agent_handler


def get_environment(config, seed):
    """
    TODO docstring
    ...
    """
    return environment.Environment(config['environment'],
                                   config['gui'],
                                   seed)
...
def point_spread(active_sensor_index, proximity,
    ...
    """
    assert function_type in ['linear', 'gauss', 'linear_normalized'], \
