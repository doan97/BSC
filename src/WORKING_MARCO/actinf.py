...
def actinf(seed, show_gui=True):
    # comp_start = comp.Start()
    #my_commands = generate_square_motor_commands()
    command_counter = 0
    random.seed(seed)
    np.random.seed(seed)
    ...
    # CONSTANTS
    NUM_ALL_STEPS = 200  # Amount of epochs of size NUM_MINI_EPOCHS
    NUM_TIME_STEPS = 10
    ...
    #save motor commands, performance only for executed commands, not thought
    plot_motor_commands = []
    plot_positions = []
    plot_performance = []
    plot_hidden_state = []
    plot_velocity_real = []
    plot_loss_to_target = []
    one_time_step_hidden_states = []
    ten_time_step_hidden_states = []
    all_time_step_hidden_states = []
    plot_time_step_counter = 0
    if c.MODE == -1:
        a1 = Agent(id_='A', color='red', init_pos=np.array([0., 1.]), gui=gui, sim=sim,
                   model_file='model_A',show_sensor_plot=True)#,less_inputs=True)

        obstacles = create_obstacles(3, gui, sim, [np.array([0., 0.5]), np.array([1.,1.5]), np.array([-1., 1.5])])
        ...
        def set_targets(from_step_):
            # All agents follow the same target
            if from_step_ % TARGET_CHANGE_FREQUENCY == 0:
                a1.create_target(TARGET_CHANGE_FREQUENCY, NUM_TIME_STEPS, position=np.array([-1., 1.8]))

            a_pos_ = a1.data.positions.get(from_step_)
            target = a1.data.actinf_targets.get(from_step_)
            distance_ = np.linalg.norm(target - a_pos_)
            plot_performance.append(distance_)

            #print(a_pos_, target)

    if c.MODE == 0:
        a1 = Agent(id_='A', color='red', init_pos=np.array([0., 1.]), gui=gui, sim=sim,
                   model_file='./saves/mode_A_final.pt')
        a2 = Agent(id_='B', color='green', init_pos=np.array([-1., 1.]), gui=gui, sim=sim,
                   model_file='./saves/mode_A_final.pt')
    ...
    if c.MODE == 1:
        ...
        def set_targets(from_step_):
            ...
            # Every time step
            a_pos = a1.data.positions.get(from_step_)
            a2.set_target(a_pos, 1, NUM_TIME_STEPS)

            # Plot the distance of B to A
            b_pos_ = a2.data.positions.get(from_step_)
            distance_ = np.linalg.norm(b_pos_ - a_pos)

            a2.performances = np.concatenate([a2.performances, [distance_]])
            a2.plot.plot([distance_])
    ...
    if c.MODE == 2:
        ...
        def set_targets(from_step_):
            ...
            a1.gui_att.update_target(a1.data.actinf_targets.get(from_step_))
            a2.set_target(1, NUM_TIME_STEPS, position=a1.data.positions.get(from_step_))
    ...
    if c.MODE == 3:
        ...
        a1 = Agent(id_='A', color='red', init_pos=pos_a1, gui=gui, sim=sim, model_file='./saves/mode_A_final.pt',
                   stopwatch=s)
        ...
        def set_targets(from_step_):
            ...
            if from_step_ % TARGET_CHANGE_FREQUENCY == 0:
                current_target = a1.data.actinf_targets.get(from_step_)
    ...
    if c.MODE == 4:
        a1 = Agent(id_='A', color='red', position_loss_weight_action_inference=0., init_pos=np.array([0., 1.]), gui=gui,
                   sim=sim, model_file='./saves/mode_A_final.pt')
        a2 = Agent(id_='B', color='green', init_pos=np.array([-1., 1.]), gui=gui, sim=sim,
                   model_file='./saves/mode_A_final.pt')
    ...
    if c.MODE == 5:
        a1 = Agent(id_='A', color='red', position_loss_weight_action_inference=0., seek_proximity=False, init_pos=np.array([-1.4, 1.]),
                   gui=gui, sim=sim, model_file='./saves/mode_A_final.pt')
        a2 = Agent(id_='B', color='green', position_loss_weight_action_inference=0., seek_proximity=True, init_pos=np.array([-1., 1.]),
                   gui=gui, sim=sim, model_file='./saves/mode_A_final.pt')
    ...
    if c.MODE == 6:
        ...
        a1 = Agent(id_='A', color='red', init_pos=init_pos, gui=gui, sim=sim, model_file='./saves/mode_A_final.pt',
                   stopwatch=s)
        agents.append(a1)
        a1.gui_att.show_simulated_positions = True
        ...
        def set_targets(from_step_):
            ...
            if from_step_ == 0:
                ...
                if a1.on_target_steps < 10:
                    ...
                    target = a1.create_target(1, NUM_TIME_STEPS)
                    a1.init_target_distance = np.linalg.norm(my_pos_ - target)
                    a1.on_target_steps = 0
                    a1.target_steps_total = 0

                my_pos_ = a1.data.positions.get_relative(0)
                target = a1.data.get_actinf_targets_block(from_step_, from_step_ + 1)[0]
                dist_ = np.linalg.norm(my_pos_ - target)
                a1.performances2D.append([dist_, a1.init_target_distance])
                a1.plot.plot([dist_ / a1.init_target_distance])
    ...
    if c.MODE == 71:
        ...
        a1 = Agent(id_='A', color='red', init_pos=agent_positions[scenario], gui=gui, sim=sim,
                   model_file='./saves/mode_A_final.pt')
        a1.gui_att.show_simulated_positions = False
        ...
        def set_targets(from_step_):
            ...
            if from_step_ == 0:
                ...
                if a1.on_target_steps < 1 and a1.target_steps_total < TARGET_CHANGE_FREQUENCY:
                    ...
                    if scenario == 4:
                        filename_ = "actinf_stage" + get_stage() + "_agent" + str(a1.id) + "_mode" + str(c.MODE)
                        a1.plot.save(filename_)
                        np.savetxt("./results/" + filename_ + ".csv", np.asarray(a1.performances2D)[:, -1, :],
                                   delimiter=";")
                        sys.exit(0)

                    new_pos = agent_positions[scenario]
                    a1.data.reset(from_step_)
                    a1.data.positions.write(np.array([new_pos]), from_step_)
            ...
            a_target_pos = a1.data.get_actinf_targets_block(from_step_, from_step_ + 1)[0]
    ...
    if c.MODE == 72:
        ...
        a_scenario = 5  # from 0 to 7
        b_scenario = (a_scenario + 2) % len(agent_positions)

        a1 = Agent(id_='A', color='red', init_pos=agent_positions[a_scenario], gui=gui, sim=sim,
                   model_file='./saves/mode_A_final.pt')
        a1.gui_att.show_simulated_positions = False

        a2 = Agent(id_='B', color='lightgreen', init_pos=agent_positions[b_scenario],
                   sensor_loss_weight_action_inference=0., gui=gui, sim=sim, model_file='./saves/mode_A_final.pt')
        a2.gui_att.show_simulated_positions = False
        a2.gui_att.show_target = False
        ...
        def set_targets(from_step_):
            ...
            if from_step_ == 0:
                ...
                if a1.on_target_steps < 1 and a1.target_steps_total < TARGET_CHANGE_FREQUENCY:
                    # Keep same target
                    a_target = a1.data.get_actinf_targets_block(from_step_, from_step_ + 1)[0]
                    a1.create_target(1, NUM_TIME_STEPS, position=a_target)
                    b_target = a2.data.get_actinf_targets_block(from_step_, from_step_ + 1)[0]
                    ...
                    a1.data.reset(from_step_)
                    a1.data.positions.write(np.array([a_new_pos]), from_step_)
                    a1.initial_state = torch.stack(a1.net.init_hidden()).data.numpy()[:, -1, :, :]
                    a1.actinf_position_predictions = []
                    a1.actinf_sensor_predictions = []
                    a1.actinf_inputs = []
                    ...
                    a2.data.reset(from_step_)
                    a2.data.positions.write(np.array([b_new_pos]), from_step_)
                    ...
                    # Calculate the initial distance of the new target to the agent
                    my_pos_ = a1.data.positions.get_relative(0)
                    init_distance = np.linalg.norm(my_pos_ - target_positions[a_scenario])
                    print('Initial distance to target =', str(init_distance))
            ...
            # Plot performance
            a_pos_ = a1.data.positions.get_relative(0)
            b_pos_ = a2.data.positions.get_relative(0)
            a_target_pos = a1.data.get_actinf_targets_block(from_step_, from_step_ + 1)[0]

            # Don't plot if the position of A is not set yet -> so if it's all zero
            if not np.any(a_pos_):
                return

            # Distance of A to target
            target_dist = np.linalg.norm(a_pos_ - a_target_pos)

            # Distance of A to B
            agent_dist = np.linalg.norm(a_pos_ - b_pos_)
            real_agent_dist = agent_dist - (2 * a1.radius)
    ...
    if c.MODE == 73:
        ...
        a2 = Agent(id_='B', color='lightgreen', init_pos=agent_positions[b_scenario], clamp_target_velocity_value=0.015,
                   gui=gui, sim=sim, model_file='./saves/mode_A_final.pt')
        a2.gui_att.show_simulated_positions = False
        a2.gui_att.show_target = False
        a2.gui_att.show_scv_targets = False
    ...
    if c.MODE == 74:
        ...
        a2 = Agent(id_='B', color='lightgreen', init_pos=b_pos, clamp_target_velocity_value=0.005, gui=gui, sim=sim,
                   model_file='./saves/mode_A_final.pt')
        a2.gui_att.show_simulated_positions = False
        a2.gui_att.show_target = False
        a2.gui_att.show_scv_targets = False
    ...
    if c.MODE == 8:
        for i in range(24):
            ...
            color = list(mcd.cnames.values())[i]
            a = Agent(id_='A' + str(i), color=color, init_pos=np.array([x, y]), is_obstacle=True, gui=gui, sim=sim,
                      model_file='./saves/mode_A_final.pt', stopwatch=s)
    ...
    if c.MODE == 81:
        for i in range(24):
            ...
            a.gui_att.show_scv_targets = False
            a.gui_att.show_target = False
            a.gui_att.show_predictions = True
            a.gui_att.show_simulated_positions = False
            a.register_agents(agents)
        ...
        def set_targets(from_step_):
            ...
            for a_ in agents:
                a_pos = a_.data.positions.get(from_step_)
                a_.create_target(1, NUM_TIME_STEPS, position=a_pos)

            # Save the distance of each agent to the closest other agent
            for a_ in agents:
                min_dist = 100.
                for other_agent in a_.other_agents:
                    a_pos = a_.data.positions.get_relative(0)
                    o_pos = other_agent.data.positions.get_relative(0)
                    dist = np.linalg.norm(a_pos - o_pos)
                    if dist < min_dist:
                        min_dist = dist

                # Now we have the distance to the closest agent
                a_.performances.append([min_dist])
    ...
    if c.MODE == 82:
        for i in range(12):
            ...
            a = Agent(
                ...
                is_obstacle=True,
                seek_proximity=False,
                position_loss_weight_action_inference=0.0,
                sensor_loss_weight_action_inference=1000.0,
                show_sensor_plot=False,
                gui=gui,
                sim=sim,
                model_file='./saves/mode_A_final.pt',
        ...
        def set_targets(from_step_):
            ...
            for a_ in agents:
                min_dist = 100.
                for o_ in a_.other_agents:
                    a_pos = a_.data.positions.get_relative(0)
                    o_pos = o_.data.positions.get_relative(0)
                    dist_ = np.linalg.norm(a_pos - o_pos)
                    if dist_ < min_dist:
                        min_dist = dist_
        ...
        def set_targets(from_step_):
            ...
            if from_step_ == 0:
                ...
                if a1.on_target_steps < 10:
                    ...
                    for o_ in obstacles:
                        o_.gui_att.update_position(np.array([100., 100.]))
                    ...
                    for o_ in obstacles:
                        o_.path_scenario = scenario_

                        if 'linear' in scenario_:
                            vel_x_ = np.random.rand() * (3. / num_time_steps_)
                            vel_x_ -= (3. / num_time_steps_) / 2.
                            vel_y_ = np.random.rand() * (2. / num_time_steps_)
                            vel_y_ -= (2. / num_time_steps_) / 2.

                            o_.path_velocities = np.array([vel_x_, vel_y_])

                        elif 'circle' in scenario_:
                            o_.path_delta_angle = np.random.rand() * (np.pi / 20.) - (np.pi / 40.)
                            o_.path_radius = np.random.rand() * 0.5 + 0.1
                            o_.path_angle = np.random.rand() * np.pi * 2
                            o_.path_center = o_.init_pos - np.array(
                                [np.math.cos(angle), np.math.sin(angle)]) * o_.path_radius

                        # Change index so that it fits to index of agent
                        o_.data.positions.change_curr_idx(from_step_ - 1)
                        o_.data.positions.append_single(o_.init_pos)
                ...
                my_pos_ = a1.data.positions.get_relative(0)
                target = a1.data.get_actinf_targets_block(from_step_, from_step_ + 1)[0]
                dist_to_target = np.linalg.norm(my_pos_ - target)

                dist_to_closest_obstacle = 100.
                for o_ in obstacles:
                    o_pos = o_.data.positions.get(from_step_)
                    dist_tmp = np.linalg.norm(my_pos_ - o_pos)
    ...
    if c.MODE == 10:
        ...
        a2 = Agent(
            ...
            gui=gui, sim=sim,
            model_file='model_A'  # ./saves/mode_T15_final.pt' #'./saves/mode_T15_final.pt'
        )
        ...
        def set_targets(from_step_):
            ...
            # Plot the distance of B to A
            b_pos_ = a2.data.positions.get(from_step_)
            distance_ = np.linalg.norm(b_pos_ - a_pos_)
    ...
    if c.MODE == 11:
        ...
        a1 = Agent(id_='A', color='red', init_pos=agent_positions[a_scenario], gui=gui, sim=sim,
                   model_file='./saves/mode_B_final.pt')
        a1.gui_att.show_simulated_positions = False
        a1.gui_att.show_predictions = False

        a2 = Agent(id_='B', color='lightgreen', init_pos=b_pos, gui=gui, sim=sim, model_file='./saves/mode_B_final.pt')
        a2.gui_att.show_simulated_positions = False
        a2.gui_att.show_target = False
        a2.gui_att.show_scv_targets = False
        a2.gui_att.show_predictions = False
        ...
        def set_targets(from_step_):
            ...
            # possible fix for missing variable reference b_pos
            b_pos_ = a2.data.positions.get(from_step_)

            if from_step_ == 0:
                # Create new target for the next target period plus the future prediction time steps
                a1.create_target(1, NUM_TIME_STEPS, position=target_positions[a_scenario])
                a2.create_target(1, NUM_TIME_STEPS, position=b_pos_)
            ...
            # Distance of A to B
            agent_dist = np.linalg.norm(a_pos - b_pos_)
            real_agent_dist = agent_dist - (2 * a1.radius)
    ...
    if c.MODE == 12:
        ...
        a1 = Agent(id_='A', color='red', init_pos=agent_positions[scenario], show_sensor_plot=False, gui=gui, sim=sim,
                   model_file='./saves/mode_A_final.pt')
        a1.gui_att.show_simulated_positions = False

        obstacles = create_obstacles(0, gui, sim, [obstacle_pos], color='lightgreen', name='B')
        # Initial step for obstacles, so that they are always 1 step further of agents
        obstacles_step(obstacles, NUM_TIME_STEPS)
    ...
    for a in agents:
        ...
        if c.OUTPUT_SENSOR_DIM > 0:
            a.data.sensors.append(a.sim.calc_sensor_data(0, 1, a, a.data.sim.sensor_dirs,
                                                         a.data.sim.borders))
    ...
    for t in range(NUM_ALL_STEPS):
        ...
        set_targets(from_step)
        #for a in agents:
            #print(a.data.motor_commands.get(from_step))

        obstacles_step(obstacles, 1)
        #for a in agents:
        #    print(t, a.data.motor_commands.get(from_step))
        for o in obstacles:
        ...
        for i in range(ACTINF_ITERATIONS):
            ...
            for input_t in range(NUM_TIME_STEPS):
                for a in agents:
                    a.predict_future(from_step, input_t)#, com = my_commands[command_counter])
                    command_counter += 1
                    plot_hidden_state.append(a.net.hidden)

            # Prediction into future is done, now calculate error, perform bwpass
            # and apply gradients to motor commands
            for a in agents:
                #one_time_step_hidden_states.append(a.net.hidden)

                if (c.MODE is 10 and a.id is 'B') or c.MODE is 82:
                    ...
                    loss = a.actinf(from_step, to_step, 1, velinf=True)
                    ...
                    tmp_position_loss_weight_actinf = a.position_loss_weight_action_inference
                    tmp_sensor_loss_weight_actinf = a.sensor_loss_weight_action_inference
                    a.position_loss_weight_actinf = 1.0
                    a.sensor_loss_weight_actinf = 0.0

                # Perform motor inference
                loss = a.actinf(from_step, to_step, LEARNING_RATE)
                plot_loss_to_target.append(loss)
        ...
        if c.SHOW_SENSOR_PLOT is True:
            ...
            if c.SHOW_SENSOR_PLOT_STEP_BY_STEP is False:
                ...
                a.sensor_plot.update(sensor_data, predictions, "t = " + str(from_step))
                plotname = a.id + "_" + "{0:4}".format(from_step).replace(" ", "0")
                a.sensor_plot.save(plotname)
                ...
                for t in range(len(a.actinf_sensor_predictions)):
                    for a in agents:
                        ...
                        if a.show_sensor_plot and c.INPUT_SENSOR_DIM > 0:
                            ...
                            # Performance issues: only draw if sensor data changed
                            a.sensor_plot.update(sensor_data, a.actinf_sensor_predictions[t],
        ...
        # Perform real step
        for a in agents:
            com, vel, pos = a.real_step(from_step)
            #command_counter += 1

            plot_velocity_real.append(vel)
            plot_motor_commands.append(com)
    ...
    all_timestep_data = {}
    all_timestep_data['coms'] = plot_motor_commands
    np.save('feasabilty_coms',plot_motor_commands)
    np.save('feasabilty_pos',plot_positions)
    all_timestep_data['perf'] = plot_performance
    all_timestep_data['hidden'] = plot_hidden_state
    all_timestep_data['vel'] = plot_velocity_real
    all_timestep_data['loss'] = plot_loss_to_target
    all_runs_data.append(all_timestep_data)

    #np.save('distance_B', performance)
    #for a in agents:
    #    np.save('hidden_{}'.format(a.id), a.hidden_states)
...
def create_obstacles(number, gui, sim, positions=None, color=None, name=None):
    ...
    for obstacle_index in range(number):
        ...
        o = Agent(id_=name, color=color, init_pos=position, lr=None, gui=gui, sim=sim, is_obstacle=True)
...
#torch.save(one_time_step_datas)
#torch.save(ten_time_step_datas)
#torch.save(all_time_step_datas)

#torch.save(fifty_runs_datas)


def generate_square_motor_commands():
    up = np.array([1,1,0,0])
    down = np.array([0,0,1,1])
    right = np.array([1,0,1,0])
    left = np.array([0,1,0,1])    #ro, lo, ru, lu

    left_move = [left for i in range(20)]
    up_move = [up for i in range(10)]
    right_move = [right for i in range(20)]
    down_move = [down for i in range(10)]
    return np.concatenate((left_move, up_move, right_move, down_move), axis=0)



for i in range(1):
    print('run ',i)
    actinf(i, show_gui=True)
torch.save(all_runs_data, 'all_runs_A')
