...
class Simulator(object):
    ...
    def update(self, previous_velocity, previous_position, motor_commands, current_agent, time_step, borders=True):
        """
    ...
    @staticmethod
    def calc_sensor_data2(from_step, to_step, agent, sensor_dirs, borders):
    ...
    @staticmethod
    def calc_sensor_data(from_step, to_step, agent, sensor_dirs, borders):
        time_steps = to_step - from_step
        sensor_data = np.zeros([time_steps, c.INPUT_SENSOR_DIM])
        for t in range(time_steps):
            my_pos = agent.data.positions.get(from_step + t)
...
def calculate_distances_to_other_agents(other_agents, my_pos, my_radius, sensor_data, time_step):
    ...
    for a in other_agents:
        other_pos = a.data.positions.get(time_step)
