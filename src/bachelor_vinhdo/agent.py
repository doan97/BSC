...
from bachelor_vinhdo.data import Data
from WORKING_MARCO.plot import Plot
from bachelor_vinhdo.simulator import point_spread
global less_inputs
...
class Agent(object):
    ...
    def predict_future(self, from_step, input_t, com=None):
        ...
        self.data.position_deltas.append_single(position_delta, 'hellp')
        self.data.positions.append_single(position, 'hello')

        self.data.accelerations.append_single(acceleration, 'hello')

        # Store the velocity of the agent as input for the next call of update
        self.data.velocities.append_single(velocity, 'hello')
    ...
    def real_step(self, from_step, com=None):
        ...
        self.data.states.append_single(torch.stack(state).data.numpy()[:, -1, :, :], 'hello')
        ...
        self.data.position_deltas.append_single(sim_position_delta, 'hello')
        self.data.positions.append_single(sim_position, 'hello')
        self.data.accelerations.append_single(sim_acceleration, 'hello')
    ...
    def save(self, model_file):
        torch.save(self.net.state_dict(), 'model_A_border')
        torch.save(self.net_sensor.state_dict(), 'model_B_border')
        torch.save(self.net_motor_only.state_dict(), 'model_C_border')
        #np.save('only_sensor_loss',self.loss_net_s), ensor)
        #np.save('marco_loss',self.loss_marco)
