...
class RingBuffer(object):
    def __init__(self, length, shape, name=None):
        self.length = length
    ...
    def write(self, input_, from_idx, that_wrong=False):
    ...
    def append_single(self, input_):
        # input has shape (shape), so add the time axis
        new_input = input_[np.newaxis, :]
        self.append(new_input)

    def append(self, input_):
        input_ = np.asarray(input_)  # Convert to numpy-array to use shape operations
        assert input_[0].shape == self.val[0].shape, "Shape of Input to Ringbuffer does not fit."

        # input must be of shape (time_steps, dim) where time_steps is the number of to be appended elements
        size = input_.shape[0]  # size = number of timesteps to add

        size_difference = (self.curr_idx + size) - self.length
        if size_difference > 0:
            # split input
            split_point = self.length - self.curr_idx
            input_first = input_[:split_point]
            input_second = input_[split_point:]
            ...
            self.val[self.curr_idx: self.curr_idx + size] = input_
